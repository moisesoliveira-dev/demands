      ...
      <ol class="flex items-center gap-1.5 text-xs text-muted-foreground">
        @for (item of items(); track $index; let last = $last) {
          @if (item.path && !last) {
            ...
            <li [class.text-foreground]="last" [class.font-medium]="last">{{ item.label }}</li>
          }
          @if (!last) {
            <li>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-muted-foreground"><polyline points="9 18 15 12 9 6"/></svg>
            </li>
          }
        }
      </ol>
